...
__all__ = ["NCSOutputLayers", "DANCEROIHeads"]

logger = logging.getLogger(__name__)


class NCSOutputLayers(nn.Module):
    @configurable
    def __init__(
        self,
...
class DANCEROIHeads(WeakROIHeads):
    """
    It's HGPS's head.
    """
    ...
    def _init_box_head(cls, cfg, input_shape):
        ...
        for k in range(1, refine_K + 1):
            box_predictor.append(
                NCSOutputLayers(cfg, box_head.output_shape, k, refine_reg[k - 1], "hgps")
            )
    ...
    def _forward_box(
        ...
        if self.training:
            ...
            for k in range(1, self.refine_K + 1):
                proposals, _ = self.sample_pseudo_targets_and_label_proposals(
                    boxes, scores, proposals, img_classes,
                    DANCEROIHeads.get_pseudo_targets,
                    suffix="_r{}".format(k),
                )
        ...
        for boxes_per_image, scores_per_image, proposals_per_image, img_classes_per_image in zip(
            ...
            for i, cls_idx in enumerate(img_classes_per_image):
                ...
                if c_i in proposals_per_image.clusters.keys():
                    clusters_per_class = proposals_per_image.clusters[c_i]
                    ...
                    gt_boxes_per_image.append(boxes_per_class[top_idx])
                    gt_scores_per_image.append(scores_per_class[top_idx])
                    gt_classes_per_image.append(cls_idx)
