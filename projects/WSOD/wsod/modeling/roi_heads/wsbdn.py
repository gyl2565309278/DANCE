...
__all__ = ["WSBDNOutputLayers", "WSBDNROIHeads"]
...
class WSBDNOutputLayers(nn.Module):
    ...
    def inference(
        ...
        scores = self.predict_weighted_probs(predictions, proposals)
...
class WSBDNROIHeads(WeakROIHeads):
    ...
    def _forward_box(
        ...
        if self.training:
            ...
            proposals, _ = self.sample_pseudo_targets_and_label_proposals(
                ...
                WSBDNROIHeads.get_pseudo_targets,
                suffix="_p0",
            )
    ...
    def get_pseudo_targets(
        ...
        pseudo_targets = []
        for boxes_per_image, scores_per_image, proposals_per_image, img_classes_per_image in zip(
            boxes, scores, proposals, img_classes
        ):
            gt_boxes_per_image = []
            ...
            for i, cls_idx in enumerate(img_classes_per_image):
                c_i = cls_idx.item()
                if c_i in proposals_per_image.clusters.keys():
                    clusters_per_class = torch.cat(proposals_per_image.clusters[c_i])
                    gt_boxes_per_image.append(
                        proposals_per_image.proposal_boxes.tensor[clusters_per_class]
                    )
                    gt_classes_per_image.append(
                        img_classes_per_image.new_full(clusters_per_class.shape, c_i)
                    )
                else:
                    boxes_per_class = boxes_per_image[:, i]
                    scores_per_class = scores_per_image[:, i]
                    top_idx = scores_per_class.argmax()
                    gt_boxes_per_image.append(boxes_per_class[top_idx].unsqueeze(0))
                    gt_classes_per_image.append(cls_idx.unsqueeze(0))
