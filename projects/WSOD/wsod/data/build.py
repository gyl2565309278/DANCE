...
def load_proposals_into_dataset(dataset_dicts, proposal_file):
    ...
    for record in dataset_dicts:
        ...
        if "clusters" in proposals:
            clusters = proposals["clusters"][i]
            for cluster in clusters:
                cluster["proposal_ids"] = np.sort((
                    cluster["proposal_ids"][:, None] == inds
                ).astype("int64").argmax(1))
            record["proposal_clusters"] = clusters
