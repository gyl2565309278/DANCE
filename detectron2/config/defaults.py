...
# If True, resize the shorter edge of the image to MIN_SIZE_{TRAIN,TEST}.
# Else, resize the longer edge of the image to MIN_SIZE_{TRAIN,TEST}.
_C.INPUT.RESIZE_SHORTEST_EDGE = True
...
# Only used in `SimpleFeaturePyramid` for ViTDet. The stride ratio for ViT's
# output scale.
_C.MODEL.FPN.SCALE_FACTORS = [4.0, 2.0, 1.0, 0.5]
...
_C.MODEL.VIT.OUT_FEATURES = ["vit12"]

_C.MODEL.VIT.IMG_SIZE = 1024
...
_C.MODEL.VIT.DROP_PATH_RATE = 0.0
_C.MODEL.VIT.USE_ABS_POS = True
_C.MODEL.VIT.USE_REL_POS = False
_C.MODEL.VIT.REL_POS_ZERO_INIT = True
_C.MODEL.VIT.WINDOW_SIZE = 0
_C.MODEL.VIT.GLOBAL_BLOCK_INDEXES = [i for i in range(_C.MODEL.VIT.DEPTH)]
_C.MODEL.VIT.RESIDUAL_BLOCK_INDEXES = []
_C.MODEL.VIT.USE_ACT_CHECKPOINT = False
_C.MODEL.VIT.PRETRAIN_IMG_SIZE = 224
_C.MODEL.VIT.PRETRAIN_USE_CLS_TOKEN = True
_C.MODEL.VIT.SQUARE_PAD = 0
...
_C.MODEL.SWIN.USE_ABS_POS = False
...
# Default base learning rate for SGD is 0.001, and for AdamW is 0.0001.
